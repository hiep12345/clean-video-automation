...
from workspace_health.models import (
    SEVERITY_ORDER,
    DoctorError,
    Issue,
    count_by,
...
class MediaRootContract:
    ...
    def physical_path(self, root: Path) -> Path:
        ...
        return (
            root.parent / self.external_directory / self.physical_directory
        ).resolve()
    ...
    *,
    ...
    if _is_reparse_point(canonical):
        ...
        if actual != expected:
            issues.append(
                Issue(
                    "MEDIA_OUTPUT_TARGET_MISMATCH",
                    "ERROR",
                    "Canonical media mount resolves to an unexpected target.",
                    path=relative_path(canonical, root),
                    details={
    ...
    elif canonical.resolve() != expected:
        issues.append(
            Issue(
                "MEDIA_OUTPUT_NOT_SHARED_MOUNT",
                "ERROR",
                "Canonical media output is not the configured shared media root.",
                path=relative_path(canonical, root),
                details={"expected_physical_root": str(expected)},
            )
        )
...
def _resolve_database_media_path(
    ...
    *,
    ...
    """Resolve supported DB path forms without duplicating ``output``.

    Runtime rows historically use either a media-family-relative path such as
    ``fb-reels/...`` or a canonical-output-relative path such as
    ``output/fb-reels/...``. Both identify the same physical bundle.
    """
    ...
    *,
    ...
    try:
        ...
        table = connection.execute(
            ...
            WHERE type='table' AND name='topic_catalog'
            """
        ).fetchone()
        if table is None:
            return [
        ...
        columns = {
            str(row[1])
            for row in connection.execute("PRAGMA table_info(topic_catalog)")
        }
    ...
    *,
    ...
    for worktree in candidates:
        ...
        for mount, target in _reparse_points_under(resolved_worktree):
            if not _paths_overlap(target, physical):
                continue
            issues.append(
                Issue(
                    "WORKTREE_SHARED_MEDIA_REPARSE_POINT",
                    "ERROR",
                    "Worktree contains a reparse point that overlaps shared "
                    "production media; cleanup must not remove this worktree.",
...
def build_report(root: Path) -> dict[str, Any]:
    """Compose the base report with production media integrity checks."""
