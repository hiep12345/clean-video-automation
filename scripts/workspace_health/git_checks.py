...
def status_entries(repo: Path) -> list[dict[str, str]]:
    ...
    for raw in output.splitlines():
        ...
        entries.append(
            {"status": status, "path": path, "classification": classify_path(path)}
        )
    return entries
...
def repository_issues(name: str, repo: Path) -> tuple[dict[str, Any], list[Issue]]:
    if not repo.exists():
        return (
    ...
    if entries:
        issues.append(
            Issue(
                ...
                path=str(repo),
                details={
                    "counts": count_by(item["classification"] for item in entries)
                },
            )
        )
