...
def duplicate_state_issues(root: Path) -> list[Issue]:
    ...
    for scan_root in (root / ".worktrees", root / "scratch"):
        if not scan_root.exists():
            continue
        try:
            for candidate in scan_root.glob("**/.agents/state/task_agent.db"):
...
def _invalid_git_states(
    connection: sqlite3.Connection, path: Path, root: Path
) -> list[Issue]:
    ...
    return [
        Issue(
            ...
            details={"task_id": task_id, "status": status, "git_state": git_state},
        )
    ...
    for (task_id,) in completed:
        keys = {
            ...
            for row in connection.execute(
                "SELECT key FROM task_evidence WHERE task_id=?", (task_id,)
            )
        }
