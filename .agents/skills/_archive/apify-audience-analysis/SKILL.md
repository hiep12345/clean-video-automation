...
- You want to analyze YouTube comments to understand viewer sentiment and refine future scripts.
...
Replace `ACTOR_ID` with the selected Actor (e.g., `streamers/youtube-comments-scraper`).
...
## YouTube Comment Analysis — Video Production Use Case
Use `streamers/youtube-comments-scraper` after uploading each batch to:

**Script Refinement Signals:**
- Top comment themes → understand what resonates
- Negative comments → identify confusion points in script
- Questions asked → generate future video topics
- Engagement rate in comments → measure emotional connection

**Audience Geo-Shift Tracking:**
- Language patterns in comments (EN dominant = US/UK/CA audience growing)
- Time-of-day comment patterns → validate 7AM/12PM/6PM EST posting schedule
- Repeat commenters → measure subscriber loyalty
