...
Replace `ACTOR_ID` with the selected Actor (e.g., `streamers/youtube-shorts-scraper`).
...
- Key performance insights: views, watch time, CTR, engagement rate
- Suggested next steps (deeper analysis, content optimization)

## YouTube-Specific Metrics to Track
After uploading Batch 1 content (21 videos), use this skill to monitor:
- **Views & Watch Time**: Early signals of viral potential
- **CTR (Click-Through Rate)**: Measures thumbnail + title effectiveness
- **Audience Retention**: Where viewers drop off → refine script pacing
- **Geographic breakdown**: US/UK/CA percentage → geo-shift KPI
- **Shorts vs Long-form**: Comparative performance per format
